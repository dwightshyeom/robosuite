...
class BoxPush(ManipulationEnv):
    def __init__(
        ...
        table_full_size=(0.8, 0.8, 0.05), # WIDENED TABLE for 1.5m pushes
        ...
        # 1. Initialize the default condition (Will be overwritten by pipeline)
        self.target_push_distance = 0.1 
        
        # Fixed spawn point on the left side of the table
        self.start_x = 0.0 
        self.start_y = 0.0
    ...
    def _load_model(self):
        ...
        # Shift robot base slightly to the left so it can reach the start and end points
        ...
        # The Dynamic Box to push
        # self.box_red = BoxObject(name="cube_red", size=[0.03, 0.03, 0.03], rgba=[1, 0, 0, 1])
        self.box_red = BoxObject(
            name="cube_red", 
            size=[0.03, 0.03, 0.03], 
            rgba=[1, 0, 0, 1],
            friction=[1e-1, 1, 1],  # <--- 1. Lower the box's friction
            # density=100                   # <--- 2. Make the box 10x lighter
        )
        
        # The 3 visual indicator bars (Width 0.6m, Depth 0.01m)
        ...
        # FUSE the bars directly into the Arena
    ...
    def _reset_internal(self):
        ...
        # Place the red box at the fixed start spot
    ...
    def _setup_observables(self):
        ...
        # =======================================================
        # CRITICAL FIX: Modality changed from "obs" to "goal"
        # =======================================================
        @sensor(modality="goal")
        def push_distance(obs_cache):
            # Formatted as a 1D array so Robomimic can append it to the dataset correctly
    ...
    def _check_success(self):
        # Calculate exactly where the target bar is located on the Y axis
        target_y = self.start_y + self.target_push_distance
        
        box_pos = self.sim.data.body_xpos[self.box_body_id]

        # Has the box been pushed far enough in Y, while staying relatively straight in X?
        y_dist_to_target = abs(box_pos[1] - target_y)
        x_dist_from_center = abs(box_pos[0] - self.start_x)
        
        # Tolerance: Box must be within 5cm of the target line, and not pushed off the sides
        reached_distance = y_dist_to_target < 0.02
        stayed_on_path = x_dist_from_center < 0.05
        stayed_on_path_z = abs(box_pos[2] - self.arena.table_top_abs[2] - 0.03) < 0.01 # Not lifted too high
